...
## Usage
```sh
./hyprc DIR PORT [-hqQv] [-b /page1:/page2:...]
```
...
the usage. `-q` sets the logging mode to error-only, while `-Q` disables output
altogether. `-v` sets the logging mode to debug with additional verbosity. `-b` allows to specify a number of disallowed resources within the
